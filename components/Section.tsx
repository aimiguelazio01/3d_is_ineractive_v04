...
import Section01Background3D from './Section01Background3D';
...
const Section: React.FC<SectionProps> = ({ data, index, lang, onExpandChange }) => {
  ...
  // Mouse tracking for glow effect (ULTRA optimized)
  const mConfig = { damping: 40, stiffness: 6000, mass: 0.01 };
  ...
  // Optimization: Don't use heavy masks on Firefox
  const effectiveMaskImage = isFirefox ? 'none' : maskImage;
  ...
  return (
    ...
    >
      {data.id === 'section_01' && (
        <motion.div
          style={{ y: yBg }}
          className="absolute inset-0 select-none z-[15] pointer-events-none"
        >
          <div className="absolute inset-0 pointer-events-auto">
            <Section01Background3D isActive={isExpanded} />
          </div>
        </motion.div>
      )}
      <motion.div
        className="w-full md:w-[40%] h-[50vh] md:h-full relative z-10 border-b md:border-b-0 md:border-r md:border-l border-white/5 overflow-hidden md:cursor-none pointer-events-none"
      ...
      <div
        className={`w-full md:w-[60%] py-16 md:py-0 relative flex items-center ${isAlternate ? 'md:justify-center' : 'md:justify-start'} overflow-hidden group pointer-events-none`}
      ...
      >
        ...
        >
          ...
          {/* Glow Layer (Masked) - Disabled on Firefox for performance */}
          {!isFirefox && (
            <motion.div
        ...
        <motion.div
          ...
          className={`relative z-20 w-full max-w-2xl ${isAlternate ? 'md:px-20' : 'md:pl-20 md:pr-8'} px-8 flex flex-col gap-6 md:gap-10 pointer-events-auto`}
        >
