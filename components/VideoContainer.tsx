...
const VideoContainer: React.FC<VideoContainerProps> = ({ poster, videoUrl }) => {
  ...
  return (
    ...
    >
      ...
      {/* Video Element with Glitch Filter - Optimized for Firefox */}
      <motion.div
        className="w-full h-full"
        style={{
          filter: (isActive && !isFirefox) ? 'url(#glitch-vibe) contrast(1.1) brightness(1.1)' : 'contrast(1.0) brightness(1.0)',
          transition: isFirefox ? 'none' : 'filter 0.3s ease-out'
        }}
        animate={isHovered ? {
          y: [0, -15, 0],
          rotate: 0,
